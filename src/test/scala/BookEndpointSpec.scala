...
import models.{Book, BookJson, User}
import org.scalatest.{AsyncWordSpec, BeforeAndAfterAll, MustMatchers}
import repository.{BookRepository, CategoryRepository, UserRepository}
import services.{ConfigService, FlywayService, MySQLService, TokenService}
...
class BookEndpointSpec extends AsyncWordSpec
  ...
  "A Book Endpoint" must {
    ...
    "return `Unauthorized` when there is an invalid token in the request" in {
      ...
      Get("/books/123123") ~> addHeader("Authorization", invalidToken) ~> bookController.routes ~>
      check {
        // Since the token is invalid, we expect it to be completed with the "Unauthorized" status
    ...
    "return the book information when the token is valid" in {
      def assertion(token: String, bookId: Long) = {
        Get(s"/books/$bookId") ~> addHeader("Authorization", token) ~> bookController.routes ~>
        check {
