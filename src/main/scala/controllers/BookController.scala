...
class BookController(val bookRepository: BookRepository, val tokenService: TokenService)(implicit ec: ExecutionContext) extends BookJson
  ...
  val routes = pathPrefix("books") {
    pathEndOrSingleSlash {
      post {
        // From its documentation: Decompresses the incoming request if it is `gzip` or `deflate`
        // compressed. Uncompressed requests are passed through untouched.
        decodeRequest {
      ...
      get {
        parameters(('title.?, 'releaseDate.as[Date].?, 'categoryId.as[Long].?, 'author.?))
          ...
          complete {
            // Pass the book search to our repository
